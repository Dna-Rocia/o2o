...
$(function(){
	...
	function getInfo(id){
		$.getJSON(infoUrl,function(data){
			if(data.success){
				...
				var optionSelected = product.product.productCategory.productCategoryId;
	...
	$('.detail-img-div').on('change','.detail-img:last-child',function(){
		if($('.detail-img').length < 6){
			$('#detail_img').append('<input type="file"  class="detail-img"/>');
		}
	});
	...
	$('#submit').click(function(){
		...
		$('.detail-img').map(function(index,item){
			...
			formData.append('productStr',JSON.stringofy(product));
			...
			formData.append('verityCodeActual',verityCodeActual);
